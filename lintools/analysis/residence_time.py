...
class Residence_time(object):
    ...
    """
    def __init__(self, topology_data_object, trajectory, start_frame_num,
                 end_frame_num, skip, topology, ligand, offset):
        self.residue_counts = {}
    ...
    def measure_residence_time(self, cutoff=3.5):
        ...
        for traj in self.trajectory:
            ...
            selection_of_protein_and_ligand = self.topology_data.universe.select_atoms(
                'protein and not name H* or (segid ' + str(self.topology_data.universe.ligand.segids[0]) +
                ' and resid ' + str(self.topology_data.universe.ligand.resids[0]) + ')')
            ...
            for frame in self.topology_data.universe.trajectory[self.start_frame_num[i]:self.end_frame_num[i]:self.skip[i]]:
                selection = selection_of_protein_and_ligand.select_atoms("protein and around 3.5 (segid " +
                                                                         str(self.topology_data.universe.ligand.segids[0]) +
                                                                         ' and resid ' + str(self.topology_data.universe.ligand.resids[0]) + ")")
                residue_list = [(atom.resname, str(atom.resid), atom.segid) for atom in selection.atoms]
                # residue_list = [(atom.resname, atom.resid, atom.segid) for atom in selection.atoms]
            ...
            assert len(self.residue_counts[i]) != 0, "Nothing to draw for this ligand (residue number: " + \
                                                     self.topology_data.universe.ligand.resids[0] + \
                                                     " on the chain " + self.topology_data.universe.ligand.segids[0] + \
                                                     ") - check the position of your ligand within the trajectory."
            i += 1
    ...
    def define_residues_for_plotting_traj(self, analysis_cutoff):
        ...
        for traj in self.residue_counts:
            self.residue_counts_fraction[traj] = {residue: float(values)/len(self.contacts_per_timeframe[traj])
                                                  for residue, values in self.residue_counts[traj].items()}
        ...
        self.topology_data.dict_of_plotted_res = {i: self.frequency[i] for i in self.frequency
                                                  if sum(self.frequency[i]) > (int(len(self.trajectory))*analysis_cutoff)}

        dict_of_plotted_res_for_output = {}
        for key in self.topology_data.dict_of_plotted_res:
            dict_of_plotted_res_for_output[(key[0], int(key[1]), key[2])] = \
                self.topology_data.dict_of_plotted_res[key]

        # Slight modification here for outputting raw data to file (Naushad AL Velgy, 2018/06/07)
        if len(self.topology_data.dict_of_plotted_res) == 0:
            print "Nothing to draw for this ligand:(residue number: " + \
            str(self.topology_data.universe.ligand.resids[0]) + " on the chain " \
                  + str(self.topology_data.universe.ligand.segids[0]) + ") - try reducing the analysis cutoff."
        else:
            with open('frequency_of_contact_raw_data.dat', 'w') as freq_file:
                freq_file.write('Resname\tResid\tChain\tFrequencies in input Trajectory order\n')
                for residue in sorted(dict_of_plotted_res_for_output, key=itemgetter(1)):
                    freq_file.write('{}\t{}\t{}\t'.format(residue[0], residue[1], residue[2]) +
                                    '\t'.join([str(np.round(freq * 100)) for freq in dict_of_plotted_res_for_output[residue]]) + '\n'
                                    )


        # assert len(self.topology_data.dict_of_plotted_res) != 0, \
        #     "Nothing to draw for this ligand:(residue number: " + \
        #     str(self.topology_data.universe.ligand.resids[0]) + \
        #     " on the chain " + str(self.topology_data.universe.ligand.segids[0]) + ") - try reducing the analysis cutoff."
